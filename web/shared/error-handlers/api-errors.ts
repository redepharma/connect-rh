...
export function parseApiError(error: unknown): ApiErrorNormalized {
  if (isApiClientError(error)) {
    ...
    const data = error.data as NestErrorShape;
    ...
    if (status === 0) {
      return {
        ...
        message: "Nao foi possivel conectar ao servidor. Verifique sua rede.",
        raw: error,
      };
    }
  ...
  if (typeof error === "object" && error !== null && "isAxiosError" in error) {
    ...
    const messages = toMessages(data?.message);
    const msg =
      joinMessages(messages) || axiosErr.message || "Erro na requisicao";
